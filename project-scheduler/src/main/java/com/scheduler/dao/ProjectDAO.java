...
public class ProjectDAO {
    ...
    public List<ScheduledEntry> fetchCurrentWeekSchedule() throws SQLException {
        ...
        LocalDate today      = LocalDate.now();
        LocalDate saturday   = SchedulingCalendar.getPlanningDay(today);
        LocalDate nextMonday = SchedulingCalendar.getNextMonday(saturday);

        String sql = "SELECT sw.project_id, p.title, sw.scheduled_day, " +
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            stmt.setDate(1, Date.valueOf(nextMonday));
